...
def _env_path() -> Path:
    # Si c’est un .exe PyInstaller, sys.executable pointe vers l’exe
    if getattr(sys, "frozen", False):
        return Path(sys.executable).with_name(".env")
    # Sinon, on est en script .py : .env à côté des fichiers
...
# Mappage noms plateformes TMDB -> tes options
...
def fetch_tmdb_movie(title: str, language: str = "fr-FR") -> dict:
    """Search TMDB and return id/title/year/rating/genres."""
    if not TMDB_API_KEY:
        raise RuntimeError("TMDB_API_KEY manquant dans .env")
...
def tmdb_poster_url(poster_path: str | None, size: str = "w92") -> str | None:
    """
    Construit une URL de poster TMDB.
    Taille sympa pour une liste de suggestions: w92.
    """
    if not poster_path:
        return None
...
def fetch_tmdb_watch_platform(movie_id: int, region: str = "FR") -> str | None:
    """TMDB watch/providers -> platform name (or None)."""
    if not TMDB_API_KEY:
        raise RuntimeError("TMDB_API_KEY manquant dans .env")
...
def upsert_movie(
    *,
    ...
    page = notion.pages.create(
        parent={"data_source_id": DATA_SOURCE_ID}, properties=props
    )
    return page["id"]
...
def list_movies(status: str | None = None, limit: int = 50) -> list[tuple[str, str]]:
    ...
    for page in res.get("results", []):
        ...
        t = title_arr[0].get("plain_text") if title_arr else "(sans titre)"
        st = props.get(PROP_STATUS, {}).get("status", {})
        s = st.get("name") if st else "(sans statut)"
        out.append((t, s))
    return out


def auto_add(title: str) -> dict:
    """TMDB -> Notion (title/year/rating/genres/platform). Returns summary."""
    ...
    return {"title": movie["title"], "platform": platform, "page_id": page_id}


def auto_add_by_tmdb_id(movie_id: int, language: str = "fr-FR") -> dict:
    """
    Ajoute un film en utilisant un ID TMDB (zéro ambiguïté).
    Récupère détails (titre/année/genres/note) + plateforme FR, puis upsert dans Notion.
    """
    if not TMDB_API_KEY:
        raise RuntimeError("TMDB_API_KEY manquant dans .env")
    ...
    return {"title": title, "platform": platform, "page_id": page_id}
